...
func _ready() -> void:
	super._ready()
	inventory.ui.connect("start_packager", Callable(self, "package"))
...
func package() -> void:
	...
	print(items)
	var product: Item = product_scene.instantiate()
	var case: Item = items.get("case")
	if items.size() < 3: return
	else: print("PACKAGING POSSIBLE")
	...
	#Remove package size from stack
